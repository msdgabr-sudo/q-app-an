...
// OFF/ON state is explicit in the existing permission flow.
...
assert(!web.includes('package=com.qiblalabs'),'Location settings handoff must never be Play-routable');
...
// Existing permission screen is intercepted only when Android Location is known OFF and geolocation permission is already granted.
...
console.log('Android Location service / Trusted GNSS cycle: PASS');
console.log('Permission granted + Location OFF -> Android settings -> launcher state refresh -> existing trusted GPS acquisition: PASS');
console.log('No Google Play fallback: PASS');
