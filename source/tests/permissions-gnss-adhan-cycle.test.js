...
assert(bootstrap.includes('permissions-onboarding.js?v=20260819-code3-gnss-handoff2'),'Bootstrap must request the fresh GNSS handoff permission asset');
assert(sw.includes("'./js/presentation/permissions-onboarding.js'"),'Permissions integration must stay in critical cache');
assert(sw.includes("VERSION='qiblaastro-3.1.0-code3-gnss-handoff-r2'"),'Service worker must evict the stale pre-handoff cache');
assert(sw.includes("GNSS_RELEASE='trusted-position-handoff-20260819-r2'"),'Service worker must advertise the trusted-position handoff release');
