...
const safeDisplay=body('safeDisplayValue');
...
assert(loop.indexOf('if(!hasTrustedGnssCoordinates())')<loop.indexOf('var nextQT=calcQibla(LAT,LON)'),'Finite trusted gate must precede Qibla calculation');
assert(update.indexOf('QT=calcQibla(LAT,LON)')<update.indexOf('refreshMdeclFromTrustedGnss'),'Computational Qibla must publish independently before optional magnetic correction');
...
assert(safeDisplay.includes("replace(/-?Infinity|NaN/g,'---')"),'Shared text output must suppress invalid numeric tokens everywhere on screen');

// Scientific equations must remain byte-identical to the approved code-3 parent.
...
console.log('GNSS post-permission handoff, finite calculation gate, pending Home display and scientific isolation: PASS');
