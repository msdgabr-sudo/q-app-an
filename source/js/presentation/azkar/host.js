...
(function(root){
  ...
  function frameContractOk(frame){try{var doc=frame&&frame.contentDocument;return !!(doc&&doc.getElementById('azkarApp')&&doc.getElementById('azHome')&&doc.getElementById('azReader'));}catch(_){return false;}}
  function mount(force){if(mounted&&!force)return true;if(loading&&!force)return false;var host=root.document&&root.document.getElementById('page-azkar');if(!host)return false;clearWatchdog();loading=true;mounted=false;removeLegacyResidue();resetHost(host);setState(host,'loading');host.setAttribute('data-presentation-source','pages/azkar.html');var frame=root.document.createElement('iframe');frame.id='qa-azkar-frame';frame.title='الأذكار — QiblaAstro';frame.src=FRAME_SRC;frame.loading='eager';frame.setAttribute('allow','autoplay');frame.style.cssText='display:block;width:100%;height:100dvh;min-height:100dvh;border:0;background:transparent;';frame.addEventListener('load',function(){clearWatchdog();if(!frameContractOk(frame)){showFailure(host,'وصل رد غير صالح بدل شاشة الأذكار الحديثة.');return;}loading=false;mounted=true;setState(host,'ready');root.dispatchEvent(new CustomEvent('qiblaastro:presentation-page-mounted',{detail:{name:'azkar',rootId:'page-azkar',source:'pages/azkar.html'}}));},{once:true});frame.addEventListener('error',function(){showFailure(host,'فشل تحميل ملف الشاشة الحديثة.');},{once:true});host.appendChild(frame);watchdog=root.setTimeout(function(){if(loading)showFailure(host,'استغرق التحميل وقتًا أطول من المتوقع.');},15000);return true;}
