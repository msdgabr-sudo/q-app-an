...
/** User-confirmed and per-install-authenticated bridge from the TWA into native Azkar reminders. */
public final class AzkarReminderActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        if ("stop".equals(mode)) { confirmStop(); return; }
    ...
    private void confirmStart() {
        new AlertDialog.Builder(this).setTitle(R.string.azkar_start_title)
                .setMessage(getString(R.string.azkar_start_message, pendingText, pendingInterval))
                .setPositiveButton(R.string.azkar_start_action, (dialog, which) -> requestPermissionThenStart())
                .setNegativeButton(R.string.azkar_cancel_action, (dialog, which) -> finish())
                .setOnCancelListener(dialog -> finish()).show();
    }
    private void confirmStop() {
        new AlertDialog.Builder(this).setTitle(R.string.azkar_stop_title).setMessage(R.string.azkar_stop_message)
                .setPositiveButton(R.string.azkar_stop_action, (dialog, which) -> { AzkarReminderScheduler.stop(this); Toast.makeText(this, R.string.azkar_stopped_toast, Toast.LENGTH_SHORT).show(); finish(); })
                .setNegativeButton(R.string.azkar_cancel_action, (dialog, which) -> finish()).setOnCancelListener(dialog -> finish()).show();
    }
    private void requestPermissionThenStart() {
        if (Build.VERSION.SDK_INT >= 33 && checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) { requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS}, REQ_NOTIFICATIONS); return; }
        startNativeReminder();
    }
    ...
    private void startNativeReminder() { AzkarReminderScheduler.start(this, pendingInterval, pendingPhrase, pendingText); Toast.makeText(this, R.string.azkar_started_toast, Toast.LENGTH_SHORT).show(); finish(); }
    private int parseInterval(String value) { try { int parsed = Integer.parseInt(value); return Math.max(MIN_INTERVAL_MINUTES, Math.min(MAX_INTERVAL_MINUTES, parsed)); } catch (Exception ignored) { return MIN_INTERVAL_MINUTES; } }
